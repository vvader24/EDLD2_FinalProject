...
# I fixed a few spelling typos, it is possible there are more 

# Anisha - Manually renaming all of these variables is a lot of work! Is it possible the data source has a file describing these conversions that you could use to make things easier? (Maybe not, I am having a similar issue with my data.)
```
...
cd %>% 
  ...
  # ANISHA - I changed the reorder without abs(), but this is just a personal preference. If you want to highlight the difference between positive/negative, maybe consider recoloring the bars if they are positive
  ggplot(aes(x = fct_reorder(country_foc, -coh_d), y = coh_d, fill = highlight)) +
